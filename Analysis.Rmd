...
```{r}
to_draw_part <- to_draw[c(1:1000),]

ggplot(aes(y = value, x = timestamps), data = to_draw_part, data_temp$waves == "alpha_absolute_1") +
  geom_line() +
  geom_smooth(span = 1, n = 15) 
```
...
```{r}
test <- add_smooth("test_loess", theta_exp_med, 0.65, 'theta_absolute_1', "1")
```
