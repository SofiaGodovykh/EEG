...
add_average <- function(df, wave){
  ...
  temp_df <- df[,names(df) %in% colnames(df)[grepl(wave,colnames(df)) & !grepl("loess",colnames(df))]]
...
```{r}
split_stages <- function(df, time_interval){
  current_time <- 0
  df$stages <- rep(0,nrow(df))
  df$stages[df$timestamps < current_time + time_interval] <- "2"
  while(TRUE){
...
next_stage <- function(df, threshold){
  #print(deriv(df$value, df$timestamps))
  spline <- predict(sm.spline(df$timestamps, df$value), df$timestamps, 1)
  diffs <- spline[2 : length(spline),] - spline[1 : (length(spline) -1),]
  signum <- sum(sign(diffs))
  if(abs(signum)/length(df) >= threshold){
    return(sign(signum))
  }
  else{
    return(0)
